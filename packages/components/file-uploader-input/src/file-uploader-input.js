...
customElementDefineHelper(
	...
	class extends TSElement {
		...
		get dropBox() {
			return html`
				<div class="${classNames.draggableInfo} ${classNames.dropBox}">
					<div class="icon-wrapper">
						<div class="icon">
							${this.dropBoxIcon}
						</div>
					</div>
					<slot name="${slotNames.dropText}">${Messages.drop_file}</slot>
				</div>
			`;
		}
		...
		get fileUploadInput() {
			return html`
				<div
				 	class="${classNames.fileUploadButton}" 
					?data-disabled="${this.disabled}"
				>
					<label>
						<slot name="${slotNames.placeholderText}">
							${Messages.placeholder[this.multiple ? "multiple" : "single"]}
						</slot>
					</label>
					<span>
						<slot name="${slotNames.buttonText}">
		...
		render() {
			return html`
				<div
					?data-rtl="${this.rtl}"
					data-size="${this.size}"
					class="${classNames.fileUploadWrapper} ${classNames.dropBox}"
				>
		...
		handleUpload(e) {
			const event = new CustomEvent(customEventNames.fileChange, {
				detail: {
					originalEvent: e,
		...
		dragEnterHandler(e) {
			if (!this.disabled && !this.disableDragAndDrop) {
				...
				if (this.dragCounter > 0) {
					this.fileUploadWrapper.classList.add(classNames.dragOverState);
				}
			}
		}
		...
		dragLeaveHandler(e) {
			if (!this.disabled && !this.disableDragAndDrop) {
				...
				this.dragCounter--;
				if (this.dragCounter === 0) {
					this.fileUploadWrapper.classList.remove(classNames.dragOverState);
				}
			}
		}
